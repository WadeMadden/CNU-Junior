...
select P_DESCRIPT, P_PRICE from PRODUCT where P_PRICE > 10.00;


-- 2. Show all invoice numbers belonging to customer Alfred Ramas (output = 1 column)
select INVOICE.inv_number from INVOICE,CUSTOMER where INVOICE.CUS_CODE = CUSTOMER.CUS_CODE and CUSTOMER.CUS_LNAME = 'Ramas' and CUSTOMER.CUS_FNAME = 'Alfred';
...
select PRODUCT.P_DESCRIPT,VENDOR.V_NAME from PRODUCT,VENDOR where PRODUCT.P_DESCRIPT like '%saw%';


-- 4. Show every unique product (description only) purchased by Kathy Smith (use Invoice Line) (output = 1 column)
select distinct PRODUCT.P_DESCRIPT from INVOICE,LINE,PRODUCT,CUSTOMER where INVOICE.inv_number = LINE.inv_number and INVOICE.CUS_CODE = CUSTOMER.CUS_CODE and CUSTOMER.CUS_LNAME = 'Smith' and CUSTOMER.CUS_FNAME = 'Kathy';
...
alter table VENDOR add ISO9000 CHAR(1) NOT NULL default 'N';
update VENDOR set ISO9000 = 'N';
select ISO9000 from VENDOR;
...
select distinct VENDOR.V_NAME,PRODUCT.P_DESCRIPT from VENDOR full outer join product on VENDOR.V_CODE = PRODUCT.V_CODE; 
...
select INVOICE.inv_number from INVOICE inner join CUSTOMER on INVOICE.CUS_CODE = CUSTOMER.CUS_CODE and CUSTOMER.CUS_AREACODE = '615';
...
select CUSTOMER.CUS_LNAME,CUSTOMER.CUS_FNAME,ROUND(SUM(LINE.LINE_UNITS*LINE.LINE_PRICE),2) from CUSTOMER left outer join INVOICE on CUSTOMER.CUS_CODE = INVOICE.CUS_CODE left outer join LINE on INVOICE.inv_number = LINE.inv_number GROUP BY CUSTOMER.CUS_LNAME,CUSTOMER.CUS_FNAME;
...
-- select CUSTOMER.CUS_LNAME,CUSTOMER.CUS_FNAME,INVOICE.inv_number from CUSTOMER left outer join INVOICE on INVOICE.inv_number is NULL and CUSTOMER.CUS_CODE = INVOICE.CUS_CODE; 
...
select cus.cus_fname, cus.cus_lname,pro.p_descript
from customer cus
left join invoice inv
on cus.cus_code = inv.cus_code
left join line lin
on lin.inv_number = inv.inv_number
left join product pro
on pro.p_code = lin.p_code
where pro.p_code is null
;
